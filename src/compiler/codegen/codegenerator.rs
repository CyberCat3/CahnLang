...
impl<'a> CodeGenerator<'a> {
    ...
    fn emit_invoke_instruction(&mut self, arg_count: u8) {
        self.emit_instruction(Instruction::Invoke);
    ...
    fn visit_expr<'b>(&mut self, expr: &Expr<'b>) -> Result<()> {
        match expr {
            ...
            Expr::Call(ce) => {
                self.emit_get_local_instruction(0); // put the current function on the stack
                println!("call expr, emitted get local 0, for putting return func on stack");

                self.emit_return_adress(self.ip);

                // get the callee on the stack
                self.visit_expr(&ce.callee)?;
                println!("call expr, visited calleee");

                // push all the arguments on the stack
                for arg in &ce.args {
                    self.visit_expr(arg)?;
                    println!("call expr, visited argument");
                }

                // invoke the function
                self.set_source_pos(ce.paren_open.pos);
                self.emit_invoke_instruction(
                    ce.args
                        .len()
                        .try_into()
                        .expect("Too many arguments, Cahn only supports up to 255"),
                );
                println!("emitted invoke");
            }
    ...
    fn visit_stmt<'b>(&mut self, stmt: &Stmt<'b>) -> Result<()> {
        Ok(match stmt {
            ...
            Stmt::FnDecl(fn_decl_stmt) => self.visit_fn_decl_stmt(fn_decl_stmt)?,
            ...
            Stmt::Return(rs) => {
                ...
                self.set_source_pos(rs.return_token.pos);
                self.emit_instruction(Instruction::Return);
            }
        })
    }

    fn visit_fn_decl_stmt<'b>(&mut self, fn_decl_stmt: &FnDeclStmt<'b>) -> Result<()> {
        let fcg = CodeGenerator::from_parent(self);
        let func = fcg.gen_function(fn_decl_stmt)?;

        self.emit_load_function_instruction(
            self.functions
                .len()
                .try_into()
                .expect("Cahn only supports up to u32::MAX functions"),
        );
        self.functions.push(func);

        self.set_source_pos(fn_decl_stmt.name.pos);
        self.declare_local(&fn_decl_stmt.name.lexeme);

        Ok(())
    }

    fn gen_program<'b>(mut self, prog_stmt: &ProgramStmt<'b>) -> Result<CahnFunction> {
    ...
    fn gen_function<'b>(mut self, fn_decl_stmt: &FnDeclStmt<'b>) -> Result<CahnFunction> {
        self.declare_anonymous_local(); // reserve first stack slot for current current function
        let fn_name = self.add_string(&fn_decl_stmt.name.lexeme);
        ...
        self.visit_block_stmt(&fn_decl_stmt.body)?;

        self.emit_instruction(Instruction::LoadNil);
        self.emit_instruction(Instruction::Return);

        let parameter_count = match fn_decl_stmt.parameters.len() {
    ...
    pub fn gen_executable<'b>(
        ...
        let main_func = fcg.gen_program(prog)?;

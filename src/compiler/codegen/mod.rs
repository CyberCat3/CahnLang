...
use crate::{
    compiler::{ast::*, lexical_analysis::{TokenType, Token}},
...
impl<'a> fmt::Debug for Local<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_fmt(format_args!("Local(name: {}, level: {})", self.identifier.lexeme, self.scope_level))
    }
}
...
impl<'a> CodeGenerator<'a> {
    ...
    fn end_scope(&mut self) {
        ...
        println!("ended scope, evicted {} locals. all locals: {:?}", locals_evicted, self.locals);
    }

    fn declare_local(&mut self, identifier: Token<'a>) {
        self.locals.push(Local {
            identifier: identifier,
            scope_level: self.scope_level,
        });
        println!("declared local {}. all locals: {:?}", identifier.lexeme, self.locals);
    }

    fn get_local(&mut self, identifier: Token<'a>) -> Result<usize> {
        ...
        match local {
            ...
            None => Err(CodeGenError::UnresolvedVariable { identifier: identifier.lexeme.into(), index: identifier.index })
        }
    }
    ...
    fn emit_load_num_instruction(&mut self, index: usize) {
        ...
        panic!("So many number constants! Cahn only supports up to {}, but got {}", u32::MAX, index);
    }
    ...
    fn emit_get_local_instruction(&mut self, index: usize) {
        ...
        panic!("So many locals! Cahn only supports up to {}, but got {}", u16::MAX, index);
    }

    fn visit(&mut self, ast: &Expr<'a>) -> Result<()> {
        ...
        match ast {
            ...
            Expr::VarDecl(vde) => {
                ...
                self.declare_local(vde.identifier);
                self.emit_instruction(Instruction::LoadNil);
            },
            
            Expr::Var(ve) => {
                let stack_offset = self.get_local(ve.identifier)?;
    ...
    fn optimize(code: Vec<u8>) -> Vec<u8> {
        ...
        while let Some(cop) = iter.next() {
            ...
            match instruction {
                Instruction::LoadConstNum | Instruction::LoadLitNum | Instruction::GetLocal | Instruction::SetLocal => {
                    code_opt.push(cop);
    ...
    pub fn gen(mut self, ast: &'a Expr) -> Result<Executable> {
        self.visit(ast)?;
